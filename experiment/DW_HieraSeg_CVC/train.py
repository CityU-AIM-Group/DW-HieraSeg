...
def CE_SPLloss_gen(loss, ClipV_weights_CE, eps=1e-10):
    loss = torch.sum(loss*ClipV_weights_CE)/(torch.sum(ClipV_weights_CE)+eps)
    return loss

def CE_SPLloss_gen1(loss, ClipV_weights_tensor1, ClipV_weights_tensor0, eps=1e-10):
...
def train_one_batch(train_list):
	...
	for i in range(3):
		CE_loss["{0}".format(i)] = criterion(Predicts["{0}".format(i)], labels_batched)
		if train_list['epoch'] >= cfg.epoch_start:
			CE_loss["{0}".format(i)] += criterion(predicts_batched["{0}".format(i)], multiclass_label["{0}".format(i)])
		'''for i in range(3):
	...
	for i in range(3):
		...
		CE_SPLloss += CE_SPLloss_gen(CE_loss["{0}".format(i)], pix_weight, eps=1e-10)
		JA_SPLloss += JA_SPLloss_gen(JA_loss["{0}".format(i)], img_weight, eps=1e-10)
		
	SPL_loss = (CE_SPLloss + JA_SPLloss)/3.
...
#	loss_weight2 = WeightLoss(weight2, normal_dim=cfg.normal_dim2, polyp_dim=cfg.polyp_dim2)
	...
	return img_weight_loss, pix_weight_loss, class_imbalance_loss, CE_SPLloss, CE_SPLloss, JA_SPLloss, img_weight, pix_weight, result
