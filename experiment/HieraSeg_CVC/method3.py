...
def train_one_batch(inputs_batched, labels_batched, net, seg_optimizer, epoch):
	...
	for i in range(1):
		...
		if epoch >= cfg.epoch_start:
			multi_CE_loss = criterion(predicts_batched["{0}".format(i)], multiclass_label["{0}".format(i)])
			CE_loss += multi_CE_loss
		
	# obtain the weight loss in classifier1
	weight0 = list(net.cls_conv_0.parameters())[0]
	loss_weight0 = WeightLoss(weight0, normal_dim=cfg.normal_dim0, polyp_dim=cfg.polyp_dim0)

	loss_weight = 0.2 * (loss_weight0).cuda()

	seg_loss = (CE_loss + JA_loss)/3. + loss_weight
	seg_optimizer.zero_grad()
	seg_loss.backward()
	seg_optimizer.step()

	return CE_loss, JA_loss, loss_weight0, loss_weight0, result
