	...
	with torch.no_grad():
		for i_batch, sample_batched in enumerate(dataloader):
			...
			for rate in cfg.TEST_MULTISCALE:
				...
				HFC0_1 = torch.argmax(predicts0, dim=1).cpu().numpy().astype(np.uint8)
				HFC1_1 = torch.argmax(predicts1, dim=1).cpu().numpy().astype(np.uint8)
				HFC2_1 = torch.argmax(predicts2, dim=1).cpu().numpy().astype(np.uint8)
