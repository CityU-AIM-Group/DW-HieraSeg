...
class LearnSPL(nn.Module):
    def __init__(self, cfg):
        ...
        self.weight_conv = nn.Sequential(
				nn.Conv2d(260, cfg.MODEL_ASPP_OUTDIM, 3, 1, padding=1,bias=True),
    ...
    def forward(self, JA_loss, lambda_JA, JA_percent, CE_loss, lambda_CE, CE_percent, feature_maps, labels):        
        ### image-level weight
    #    print(JA_loss.shape)
    #    x_percent = self.JA_percent_emb((JA_percent*100*torch.ones(JA_loss.size(0))).int().long().cuda())
        ...
        x = torch.cat((JA_loss, x_percent, JA_loss-x_lambda), dim=1)
    ...
    #    print(feature_maps.shape, labels.shape, CE_loss.shape)
        ...
        CE_percent = imgresize(torch.unsqueeze(CE_percent, 1))
        lambda_CE = imgresize(torch.unsqueeze(lambda_CE, 1))
        feature_cat = torch.cat([feature_maps, labels, CE_loss, CE_percent, CE_loss-lambda_CE],1)
