...
#        S2 = S1 + self.conv1(S1 + B1)
#        B2 = B1 + self.conv1_1(S1 * B1)

#        S3 = S2 + self.conv2(S2 + B2)
#        B3 = B2 + self.conv2_1(S2 * B2)

        S2 = S1 + S1 * F.sigmoid(self.conv1(S1 + B1))
        B2 = B1 + B1 * F.sigmoid(self.conv1_1(S1 * B1))

        S3 = S2 + S2 * F.sigmoid(self.conv2(S2 + B2))
#        B3 = B2 + B2 * F.sigmoid(self.conv2_1(S2 * B2))
...
class BANet(nn.Module):
	def __init__(self, cfg):
		...
		self.BAA2 = BAA(cfg, 1024, cfg.MODEL_SHORTCUT_DIM)
		self.BAA1 = BAA(cfg, 512, cfg.MODEL_SHORTCUT_DIM)
		self.BAA0 = BAA(cfg, 256, cfg.MODEL_SHORTCUT_DIM)
