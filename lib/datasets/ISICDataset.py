...
class ISICDataset(Dataset):
    def __init__(self, dataset_name, cfg, period, aug=False):
        ...
        if 'train' in self.period: 
            ...
            self.randomsharpness = RandomSharpness()       
            self.randomcolor = RandomColor()       
            if cfg.DATA_RANDOMCROP > 0:
            #    self.randomcrop = RandomCrop(cfg.DATA_RANDOMCROP)
                self.randomcrop = CenterCrop(cfg.DATA_RANDOMCROP)
    ...
    def __getitem__(self, idx):
        ...
        if 'train' in self.period:
            ...
            sample = self.randomcontrast(sample)
            sample = self.randombrightness(sample)
            sample = self.randomsharpness(sample)
            sample = self.randomcolor(sample)
    ...
    def decode_labels(self, mask, label_colours, num_classes=21):
        ...
        outputs = np.uint8(np.array(img))
        return outputs

    def save_result_HFC(self, result_list, model_id):
        """Save test results
        ...
        """
        #color1 = [[1,148,58], [0,185,75], [71,210,122], [128,0,79], [147,0,85], [159,0,96], [169,31,118], [196,64,141], [207,163,168], [178,30,68], [198,62,96], [208,106,129], [219,109,215]]
        #color2 = [[1,148,58], [0,185,75], [128,0,79], [147,0,85], [159,0,96], [169,31,118], [196,64,141], [207,163,168], [178,30,68], [198,62,96], [208,106,129], [219,109,215]]
        #color3 = [[1,148,58], [128,0,79], [147,0,85], [159,0,96], [169,31,118], [196,64,141], [207,163,168], [178,30,68], [198,62,96], [208,106,129], [219,109,215]]
        color1 = [[58,148,1], [75,185,0], [122,210,71], [79,0,128], [85,0,147], [96,0,159], [118,31,169], [141,64,196], [168,163,207], [68,30,178], [96,62,198], [129,106,208], [215,109,219]]
        color2 = [[58,148,1], [75,185,0], [79,0,128], [85,0,147], [96,0,159], [118,31,169], [141,64,196], [168,163,207], [68,30,178], [96,62,198], [129,106,208], [215,109,219]]
        color3 = [[58,148,1], [79,0,128], [85,0,147], [96,0,159], [118,31,169], [141,64,196], [168,163,207], [68,30,178], [96,62,198], [129,106,208], [215,109,219]]
        i = 1
        folder_path = os.path.join(self.rst_dir,'%s_%s_cls'%(model_id,self.period))
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        for sample in result_list:
            ...
            # p = self.__coco2voc(sample['predict'])
            cv2.imwrite(file_path, sample['predict'])
            cv2.imwrite(name+'hfcm0_1.png', sample['hfcm0_1'])
            cv2.imwrite(name+'hfcm1_1.png', sample['hfcm1_1'])
            cv2.imwrite(name+'hfcm2_1.png', sample['hfcm2_1'])
            print('%s'%sample['name'])
            print(np.unique(sample['hfcm0']))
            print(np.unique(sample['hfcm1']))
            hfcm0 = self.decode_labels(sample['hfcm0'], color1, num_classes=12)
            hfcm1 = self.decode_labels(sample['hfcm1'], color2, num_classes=6)
            hfcm2 = self.decode_labels(sample['hfcm2'], color3, num_classes=2)
            cv2.imwrite(name+'hfcm0.png', hfcm0)
            cv2.imwrite(name+'hfcm1.png', hfcm1)
            cv2.imwrite(name+'hfcm2.png', hfcm2)
            # print('[%d/%d] %s saved'%(i,len(result_list),file_path))
            i+=1
